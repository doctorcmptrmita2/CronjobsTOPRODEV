...
class StoreJobRequest extends FormRequest
{
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            $maxJobs = $plan?->max_jobs ?? 5;
            $minInterval = $plan?->min_interval_minutes ?? 15;

            if ($this->isMethod('post') && $user->jobs()->count() >= $maxJobs) {
                $validator->errors()->add('name', __('Plan limit reached. You can create up to :count jobs.', ['count' => $maxJobs]));
            }
